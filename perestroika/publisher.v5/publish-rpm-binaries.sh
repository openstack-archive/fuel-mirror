...
SIGUL_USER=${SIGUL_USER:-"sigul"}

main() {
  if [ -n "${SIGKEYID}" ] ; then
      # Is sigul availble and signed key exist
      check-sigul "${SIGKEYID}" && USE_SIGUL=true
      if [ "${USE_SIGUL}" = "true" ] ; then
          # Use sigul for sign
          sigul -u "${SIGUL_USER}" get-public-key "${SIGKEYID}" --password > RPM-GPG-KEY
      else
          # Use local sign
          check-gpg || :
  ...
  for binary in ${BINRPMLIST} ${BINSRCLIST} ; do
      ...
      if [ -n "${SIGKEYID}" ] ; then
        # rpmsign requires pass phrase. use `expect` to skip it
          if [ "${USE_SIGN}" = "true" ] ; then
              mv "${binary}" "unsign_${binary}"
              sigul -u "${SIGUL_USER}" sign-rpm -o ${binary} "${SIGKEYID}" "unsign_${binary}"
          else
              LANG=C expect <<EOL
...
EOL
  ...
  if [ -n "${SIGKEYID}" ] ; then
      ...
      if [ "${USE_SIGUL}" = true ] ; then
          sigul -u "${SIGUL_USER}" sign-data -o "${LOCAL_REPO_PATH}/x86_64/repodata/repomd.asc" "${SIGKEYID}" "${LOCAL_REPO_PATH}/x86_64/repodata/repomd.xml"
          sigul -u "${SIGUL_USER}" sign-data -o "${LOCAL_REPO_PATH}/Source/repodata/repomd.asc" "${SIGKEYID}" "${LOCAL_REPO_PATH}/Source/repodata/repomd.xml"
      else
