...
main() {
  if [ -n "${SIGKEYID}" ] ; then
      ...
      if [ "${USE_SIGUL}" = "true" ] ; then
          # Use sigul for sign
          sigul-expect $SIGKEYID $SIGUL_USER get-public-key ${SIGKEYID} | sed -rn '/^-----BEGIN/,/BLOCK-----$/ s/\r// p' > RPM-GPG-KEY
      else
          # Use local sign
          check-gpg || :
        ...
        # rpmsign requires pass phrase. use `expect` to skip it
          if [ "${USE_SIGUL}" = "true" ] ; then
              mv "${binary}" "unsign_${binary}"
               sigul-expect $SIGKEYID $SIGUL_USER $KEY_PASSPHRASE sign-rpm -o ${binary} ${SIGKEYID} "unsign_${binary}"
          else
              LANG=C expect <<EOL
  ...
  if [ -n "${SIGKEYID}" ] ; then
      ...
      if [ "${USE_SIGUL}" = true ] ; then
          for TYPE in x86_64 Source ; do
              sigul-expect $SIGKEYID $SIGUL_USER $KEY_PASSPHRASE sign-data -o "${LOCAL_REPO_PATH}/${TYPE}/repodata/repomd.asc" "${SIGKEYID}" "${LOCAL_REPO_PATH}/${TYPE}/repodata/repomd.xml"
          done
      else
