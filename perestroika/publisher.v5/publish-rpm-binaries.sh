...
main() {
  if [ -n "${SIGKEYID}" ] ; then
      ...
      if [ "${USE_SIGUL}" = "true" ] ; then
          # Use sigul for sign
          printf '%s\0' "$SIGUL_ADMIN_PASSWD" | sigul --batch -u $SIGUL_USER get-public-key ${SIGKEYID} > RPM-GPG-KEY
      else
          # Use local sign
          check-gpg || :
      ...
      if [ -n "${SIGKEYID}" ] ; then
        # rpmsign requires pass phrase. use `expect` to skip it
          if [ "${USE_SIGUL}" = "true" ] ; then
              ...
              printf '%s\0' "$KEY_PASSPHRASE" | sigul --batch -u $SIGUL_USER sign-rpm -o ${binary} ${SIGKEYID} "unsign_${binary}"
          else
              LANG=C expect <<EOL
