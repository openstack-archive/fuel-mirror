  ...
  if [ -n "${SIGKEYID}" ] ; then
      ...
      if [ "${USE_SIGUL}" = true ] ; then
          for TYPE in x86_64 Source ; do
              retry -c4 -s1 _sigul "$KEY_PASSPHRASE" -u $SIGUL_USER sign-data-armored -o "${LOCAL_REPO_PATH}/${TYPE}/repodata/repomd.asc" "${SIGKEYID}" "${LOCAL_REPO_PATH}/${TYPE}/repodata/repomd.xml"
          done
      else
