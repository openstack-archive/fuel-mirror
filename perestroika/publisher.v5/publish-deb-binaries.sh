...
main() {
    ...
    if [ "${GERRIT_CHANGE_STATUS}" = "NEW" ] ; then
        ...
        LOCAL_REPO_PATH="${REPO_BASE_PATH}/${DEB_REPO_PATH}"
        URL_PREFIX=""
    ...
    if [ ! -d "${CONFIGDIR}" ] ; then
        ...
        job_lock "${CONFIGDIR}.lock" wait 3600
            ...
            for dist_name in "${DEB_DIST_NAME}"          \
                ...
                debug "Processing distribution ${dist_name}"
    ...
    echo
    echo "DEBUG: Given directives:"
    echo "DEBUG: IS_UPDATES=${IS_UPDATES}"
    echo "DEBUG: IS_HOLDBACK=${IS_HOLDBACK}"
    echo "DEBUG: IS_SECURITY=${IS_SECURITY}"
    echo "DEBUG: IS_RESTRICTED=${IS_RESTRICTED}"
    echo "DEBUG: IS_DOWNGRADE=${IS_DOWNGRADE}"
    echo
    ...
    if [ -z "${LOCAL_DEB_COMPONENT}" ] ; then
        LOCAL_DEB_COMPONENT="main"
    fi
    ...
    echo
    echo "DEBUG: After applying all workflow overrides will publish with"
    echo "DEBUG: LOCAL_DEB_DIST_NAME=${LOCAL_DEB_DIST_NAME}"
    echo "DEBUG: LOCAL_DEB_COMPONENT=${LOCAL_DEB_COMPONENT}"
    echo
