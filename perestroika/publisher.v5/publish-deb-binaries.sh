...
main() {
    ...
    if [ ! -d "${CONFIGDIR}" ] ; then
        ...
        job_lock "${CONFIGDIR}.lock" wait 3600
            ...
            for dist_name in "${DEB_DIST_NAME}"          \
                ...
                info "Processing distribution ${dist_name}"
    ...
    echo
    echo "INFO: Given directives:"
    echo "INFO: IS_UPDATES=${IS_UPDATES}"
    echo "INFO: IS_HOLDBACK=${IS_HOLDBACK}"
    echo "INFO: IS_SECURITY=${IS_SECURITY}"
    echo "INFO: IS_RESTRICTED=${IS_RESTRICTED}"
    echo "INFO: IS_DOWNGRADE=${IS_DOWNGRADE}"
    echo
    ...
    echo
    echo "INFO: After applying all workflow overrides will publish with"
    echo "INFO: LOCAL_DEB_DIST_NAME=${LOCAL_DEB_DIST_NAME}"
    echo "INFO: LOCAL_DEB_COMPONENT=${LOCAL_DEB_COMPONENT}"
    echo
