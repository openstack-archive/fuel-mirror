      ...
      if [ ! -d "${CONFIGDIR}" ] ; then
          ...
          for dist_name in ${DEB_DIST_NAME} ${DEB_PROPOSED_DIST_NAME} ${DEB_UPDATES_DIST_NAME} \
              ...
              if [ "${USE_SIGUL}" = "true" ] ; then
                  printf '%s\0' "$KEY_PASSPHRASE" | sigul --batch -u $SIGUL_USER sign-data -o "${release_file}.gpg" ${SIGKEYID} ${release_file}
              else
                  [ -n "${SIGN_STRING}" ] \
...
#  if [ -n "${SIGN_STRING}" ] ; then
      if [ "${USE_SIGUL}" = "true" ] ; then
          ...
          printf '%s\0' "$KEY_PASSPHRASE" | sigul --batch -u $SIGUL_ADMIN get-public-key ${SIGKEYID} > "${pub_key_file}.tmp"
      else
