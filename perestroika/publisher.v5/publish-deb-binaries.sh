...
main() {
    ...
    # note: we have convention that DEB_UPDATES_DIST_NAME DEB_UPDATES_COMPONENT
    #       could be provided for defining repo for updates. But this is not implemented.
    ...
    if [ "${IS_UPDATES}" = 'true' ] ; then
        ...
        # It's possible to provide another distribution name for package
        if [ -n "${DEB_PROPOSED_DIST_NAME}" ] ; then

            LOCAL_DEB_DIST_NAME="${DEB_PROPOSED_DIST_NAME}"

        else
            warning "IS_UPDATES is set to true, but no DEB_PROPOSED_DIST_NAME is provided"
        fi

        # It's possible to provide another component name for package
        if [ -n "${DEB_PROPOSED_COMPONENT}" ] ; then

            LOCAL_DEB_COMPONENT="${DEB_PROPOSED_COMPONENT}"

        else
            warning "IS_UPDATES is set to true, but no DEB_PROPOSED_COMPONENT is provided"
        fi
    ...
    if [ "${IS_HOLDBACK}" = 'true' ] ; then
        ...
        # It's possible to provide another distribution name for package
        if [ -n "${DEB_HOLDBACK_DIST_NAME}" ] ; then

            LOCAL_DEB_DIST_NAME="${DEB_HOLDBACK_DIST_NAME}"

        else
            warning "IS_HOLDBACK is set to true, but no DEB_HOLDBACK_DIST_NAME is provided"
        fi

        # It's possible to provide another component name for package
        if [ -n "${DEB_HOLDBACK_COMPONENT}" ] ; then

            LOCAL_DEB_COMPONENT="${DEB_HOLDBACK_COMPONENT}"

        else
            warning "IS_HOLDBACK is set to true, but no DEB_HOLDBACK_COMPONENT is provided"
        fi
    fi
    ...
    if [ "${IS_SECURITY}" = 'true' ] ; then
        ...
        # It's possible to provide another distribution name for package
        if [ -n "${DEB_SECURITY_DIST_NAME}" ] ; then

            LOCAL_DEB_DIST_NAME="${DEB_SECURITY_DIST_NAME}"

        else
            warning "IS_SECURITY is set to true, but no DEB_SECURITY_DIST_NAME is provided"
        fi

        # It's possible to provide another component name for package
        if [ -n "${DEB_SECURITY_COMPONENT}" ] ; then

            LOCAL_DEB_COMPONENT="${DEB_SECURITY_COMPONENT}"

        else
            warning "IS_SECURITY is set to true, but no DEB_SECURITY_COMPONENT is provided"
        fi
    fi
    ...
    job_lock "${CONFIGDIR}.lock" wait 3600
        ...
        #       some useful checks
        #
        # note: looks like this case is useful for republishing only, because
