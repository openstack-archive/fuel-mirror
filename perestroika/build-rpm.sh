...
' )
  ...
  EXTRAREPO="${EXTRAREPO}|repo2,http://${REMOTE_REPO_HOST}/${RPM_SECURITY_REPO_PATH}/x86_64"
  if [ "$IS_HOTFIX" == 'true' ] ; then
      EXTRAREPO="${EXTRAREPO}|repo3,http://${REMOTE_REPO_HOST}/${RPM_HOTFIX_REPO_PATH}/x86_64"
      EXTRAREPO="${EXTRAREPO}|repo4,http://${REMOTE_REPO_HOST}/${RPM_UPDATES_REPO_PATH}/x86_64"
  elif [ "$IS_SECURITY" == "true" ] ; then
      EXTRAREPO="${EXTRAREPO}|repo3,http://${REMOTE_REPO_HOST}/${RPM_UPDATES_REPO_PATH}/x86_64"
  elif [ "$IS_UPDATES" == 'true' ] ; then
      EXTRAREPO="${EXTRAREPO}|repo3,http://${REMOTE_REPO_HOST}/${RPM_PROPOSED_REPO_PATH}/x86_64"
  fi

  if [ "$GERRIT_CHANGE_STATUS" == "NEW" ] && [ -n "$LP_BUG" -o -n "$CUSTOM_REPO_ID" ] ; then
      [ -z "RPM_REQUEST_HOTFIX_REPO_PATH" ] && local RPM_REQUEST_HOTFIX_REPO_PATH=$RPM_HOTFIX_REPO_PATH
      [ -z "RPM_REQUEST_SECURITY_REPO_PATH" ] && local RPM_REQUEST_SECURITY_REPO_PATH=$RPM_SECURITY_REPO_PATH
      [ -z "RPM_REQUEST_PROPOSED_REPO_PATH" ] && local RPM_REQUEST_PROPOSED_REPO_PATH=$RPM_PROPOSED_REPO_PATH
      [ -z "RPM_REQUEST_OS_REPO_PATH" ] && local RPM_REQUEST_OS_REPO_PATH=$RPM_OS_REPO_PATH
      if [ "$IS_HOTFIX" == 'true' ] ; then
