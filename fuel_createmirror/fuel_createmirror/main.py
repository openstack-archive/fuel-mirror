...
def setup_log():
    ...
    if options.log.filename:
        logger.addHandler(logging.FileHandler(options.log.filename))
    else:
        # by default write to stderr
        logger.addHandler(logging.StreamHandler())

    logger.setLevel(options.log.level.upper())
...
def entry_point(func):
    """Makes entry point."""
    def wrapper():
        with open("/var/run/fuel_createmirror.pid", "w") as lock:
            lock.write(str(os.getgid()))
