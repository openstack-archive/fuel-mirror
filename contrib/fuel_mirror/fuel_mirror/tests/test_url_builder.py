    ...
    def test_get_repo_config(self):
        self.assertEqual(
            ...
            self.builder.get_repo_config(
                "ubuntu",
                "http://localhost/ubuntu",
                "{version}-updates main restricted",
                version="trusty"
            )
        )
...
class TestYumUrlBuilder(base.TestCase):
    ...
    def test_join_url(self):
        self.assertEqual(
            ...
            self.builder.get_repo_url("http://localhost/centos", "os")
        )
        self.assertEqual(
            "http://localhost/centos/os",
            self.builder.join_url("http://localhost/centos/", "os")
        )
        ...
        self.assertEqual(
            {
                "name": "centos",
                "type": "rpm",
                "uri": "http://localhost/centos/6/os/$basearch"
            },
            self.builder.get_repo_config(
                "centos",
                "http://localhost/centos/{version}",
                "os",
                version=6
            )
        )
        self.assertEqual(
            {
                "name": "centos-updates",
                "type": "rpm",
                "uri": "http://localhost/centos/updates/$basearch"
            },
            self.builder.get_repo_config(
                "centos",
