...
class TestAptUrlBuilder(base.TestCase):
    ...
    def test_set_repo_url(self):
        new_data = self.builder.set_repo_url(
            self.repo_data, "http://ubuntu.com"
        )
        self.assertIsNot(new_data, self.repo_data)
        self.assertEqual("http://localhost/ubuntu", self.repo_data["uri"])
        self.assertEqual("http://ubuntu.com", new_data["uri"])
        new_data.pop("uri")
        repo_data = self.repo_data.copy()
        repo_data.pop("uri")
        self.assertEqual(repo_data, new_data)
...
class TestYumUrlBuilder(base.TestCase):
    ...
    def test_set_repo_url(self):
        new_data = self.builder.set_repo_url(
            self.repo_data, "http://centos.com"
        )
        self.assertIsNot(new_data, self.repo_data)
        self.assertEqual("http://localhost/os/x86_64", self.repo_data["uri"])
        self.assertEqual("http://centos.com/os/x86_64", new_data["uri"])
        new_data.pop("uri")
        repo_data = self.repo_data.copy()
        repo_data.pop("uri")
        self.assertEqual(repo_data, new_data)
