...
class RepoUrlBuilder(object):
    ...
    @classmethod
    def set_repo_url(cls, repo_data, new_url):
        """Updates the uri component for repo_data.
        :param repo_data: the repositories`s meta data
        :param new_url: the new url
        :return: the repo_data with updated uri
        """
...
class AptRepoUrlBuilder(RepoUrlBuilder):
    ...
    @classmethod
    def set_repo_url(cls, repo_data, new_url):
        repo_data_new = repo_data.copy()
        repo_data_new["uri"] = new_url
        return repo_data_new
...
class YumRepoUrlBuilder(RepoUrlBuilder):
    ...
    @classmethod
    def set_repo_url(cls, repo_data, new_url):
        base_url, repo_name, repo_arch = cls.split_url(repo_data["uri"], 2)
        repo_data_new = repo_data.copy()
        repo_data_new['uri'] = "/".join((new_url, repo_name, repo_arch))
        return repo_data_new
