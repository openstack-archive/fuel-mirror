...
from packetary.drivers import apt_driver
...
class TestAptDriver(base.TestCase):
    @classmethod
    def setUpClass(cls):
        super(TestAptDriver, cls).setUpClass()
        cls.driver = apt_driver.AptRepositoryDriver()
    ...
    def test_update_suite_index(
        ...
        utils.get_size_and_checksum_for_files.return_value = (
            (
                ...
                10,
                (k + "_value" for k in apt_driver._CHECKSUM_METHODS)
            )
            for name in apt_driver._REPOSITORY_FILES
        )
        ...
        for k in apt_driver._CHECKSUM_METHODS:
