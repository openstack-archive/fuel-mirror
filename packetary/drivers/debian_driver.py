...
class DebRepositoryDriver(RepositoryDriverBase):
    ...
    def rebuild_repository(self, repository, packages):
        ...
        metadir = utils.get_path_from_url(self._get_url_of_metafile(
            repository, ""
        ))
        utils.ensure_dir_exist(metadir)
        index = os.path.join(metadir, "Packages")
        index_gz = os.path.join(metadir, "Packages.gz")
        count = 0
    ...
    def clone_repository(self, connection, repository, destination,
        ...
        self.logger.info("clone repository %s to %s", repository, metadir)
        utils.ensure_dir_exist(metadir)
        ...
        with closing(open(os.path.join(metadir, "Release"), "wb")) as fd:
            release.dump(fd)
        # creates default files
        open(os.path.join(metadir, "Packages"), "ab").close()
        gzip.open(os.path.join(metadir, "Packages.gz"), "ab").close()
        return clone
    ...
    def _add_to_release(release, repository):
        """Adds repository information to index file.
    ...
    def _add_files_to_release(release, basepath, files):
        """Adds information about meta files to release."""
