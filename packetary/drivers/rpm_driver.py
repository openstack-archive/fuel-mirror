...
class RpmRepositoryDriver(RepositoryDriverBase):
    ...
    def get_repository(self, connection, url, arch, consumer):
        ...
        Note:
        The URL has following format: 'base[//repo_dir]'.
        The 'repo_dir' is the part of url, that will be used when
        construct local path on mirroring. For example:
        'http://localhost//centos/os/x86_64 is copied to '/var/mirror/'
        The final local path will be '/var/mirror/centos/os/x86_64'.

        :param connection: the connection manager instance
        :param url: The repository URL
        :param arch: the target architecture
        :param consumer: the callable to handle created repository
        """
        baseurl = Template(url).substitute(basearch=arch)
        name = utils.get_path_from_url(baseurl, False).partition("//")[-1]
        consumer(Repository(
            name=name,
            url=baseurl + "/",
            architecture=arch,
    ...
    def fork_repository(self, connection, repository, destination,
        ...
        destination = os.path.join(destination, repository.name, "")
