...
Version: %{version}
Release: %{release}
Source0: fuel-mirror-%{version}.tar.gz
...
install -m 755 %{_builddir}/%{name}-%{version}/contrib/fuel_mirror/etc/config.yaml %{buildroot}/etc/fuel-mirror/config.yaml
...
%attr(0644,root,root) /etc/fuel-mirror/config.yaml
