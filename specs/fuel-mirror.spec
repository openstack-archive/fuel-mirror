...
cd %{_builddir}/%{name}-%{version}/contrib/fuel-mirror && python setup.py build
...
cd %{_builddir}/%{name}-%{version}/contrib/fuel-mirror && python setup.py install --single-version-externally-managed -O1 --root=$RPM_BUILD_ROOT --record=%{_builddir}/%{name}-%{version}/contrib/fuel-mirror/INSTALLED_FILES

mkdir -p %{buildroot}/etc/fuel-mirror
mkdir -p %{buildroot}/usr/bin/
mkdir -p %{buildroot}/usr/share/fuel-mirror
install -m 755 %{_builddir}/%{name}-%{version}/contrib/fuel-mirror/fuel-createmirror %{buildroot}/usr/bin/fuel-createmirror

%clean
rm -rf $RPM_BUILD_ROOT

%files -f %{_builddir}/%{name}-%{version}/contrib/fuel-mirror/INSTALLED_FILES
%defattr(0755,root,root)
%{buildroot}/usr/bin/fuel-createmirror
