...
# Description:
#    Get Gerrit project name by package name
# Input:
#     PACKAGE_NAME
# Output:
#     GERRIT_PROJECT
...
get_gerrit_properties () {
    ...
    ssh -p $GERRIT_PORT ${GERRIT_USER}@${GERRIT_HOST} gerrit query --format=$GERRIT_QUERY_FORMAT --current-patch-set project:{$GERRIT_PROJECT} before:{$TIME_BEFORE} status:merged limit:$GERRIT_QUERY_LIMIT > $GERRIT_COMMIT_INFO_FILE_PATH
